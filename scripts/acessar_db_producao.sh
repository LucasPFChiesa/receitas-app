...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  echo "Uso: bash scripts/acessar_db_producao.sh [SQL]"
  echo "Abre o banco SQLite de producao na VM."
  echo
  echo "Exemplos:"
  echo "  bash scripts/acessar_db_producao.sh"
  echo "  bash scripts/acessar_db_producao.sh '.tables'"
  echo "  bash scripts/acessar_db_producao.sh 'SELECT * FROM schema_migrations;'"
  echo
  echo "Para outra VM:"
  echo "  VM_HOST=IP VM_USER=usuario bash scripts/acessar_db_producao.sh"
  exit 0
fi
